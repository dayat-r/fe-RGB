...
@Component({
  selector: 'app-dialog-add-merk',
  templateUrl: './dialog-add-merk.component.html',
  styleUrls: ['./dialog-add-merk.component.css']
})
export class DialogAddMerkComponent implements OnInit {
  ...
  constructor(
    ...
    private dialogRef : MatDialogRef<DialogAddMerkComponent>,
    ...
    if(this.data.name){
      ...
      this.restApi.saveMerk(formData).then((res:any)=>{
        ...
        this.dialogRef.close({
          id : this.data.kodeMerk,
          name : this.data.name
        })

      })
    }else{
      this.snackbar.open("Lengkapi data nama merk terlebih dahulu","",{
        duration: 2000
      })
    }
